...
export function DashboardFunded() {
  ...
  const funded = summary.challenge.isFunded;
      ...
      <section className={styles.commandGrid}>
        <article className={`${styles.panel} ${styles.challengePanel}`}>
          <div className={styles.panelHeader}>
            <div>
              <span>{funded ? "ACTIVE FUNDED ACCOUNT" : "ACTIVE CHALLENGE"}</span>
            ...
            {challenge && (
              <div className={styles.challengeBadges}>
                ...
                <span className={styles.statusBadge}>{funded && summary.challenge.payoutEligible ? "PAYOUT ELIGIBLE" : challengeStatusLabel(challenge.status)}</span>
              </div>
            )}
          </div>
            ...
            <div className={styles.challengeBody}>
                ...
                {funded ? (
                  <>
                    <div>
                      ...
                      <strong>{money.format(summary.challenge.payoutAvailable)}</strong>
                    </div>
                    <div>
                      <span>EST. TAKE-HOME</span>
                      <strong>{money.format(summary.challenge.estimatedPayout)}</strong>
                    </div>
                    <div>
              ...
              <div className={styles.barGroup}>
                <div className={styles.barLabel}>
                  <span>{funded ? "Payout readiness" : "Profit target"}</span>
                  <b>{number.format(funded ? summary.challenge.payoutReadinessPct : summary.challenge.targetProgressPct)}%</b>
                </div>
                <div className={styles.progressTrack}>
                  <i style={{ width: `${Math.min(100, funded ? summary.challenge.payoutReadinessPct : summary.challenge.targetProgressPct)}%` }} />
                </div>
              ...
              <Link href="/challenges" className={styles.panelLink}>
                {funded ? "OPEN FUNDED TRACKER →" : "OPEN CHALLENGE PLANNER →"}
              </Link>
            </div>
          ) : (
