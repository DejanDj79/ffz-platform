...
function parseOptionalNumber(raw: string) {
  ...
  if (!Number.isFinite(value) || value <= 0) {
    throw new Error(
      "Optional price values must be greater than 0.",
    );
  }
...
export function TradeJournal() {
  const [trades, setTrades] =
    useState<TradeApiModel[]>([]);

  const [challenges, setChallenges] =
    useState<Challenge[]>([]);

  const [draft, setDraft] =
    useState<Draft>(blankDraft);

  const [editingId, setEditingId] =
    useState<string | null>(null);

  const [existingAttachments, setExistingAttachments] =
    useState<TradeAttachmentApiModel[]>([]);

  const [queuedImages, setQueuedImages] =
    useState<QueuedImage[]>([]);
  ...
  const [viewingTrade, setViewingTrade] =
    useState<TradeApiModel | null>(null);

  const [viewAttachments, setViewAttachments] =
    useState<TradeAttachmentApiModel[]>([]);
  ...
  const [lightbox, setLightbox] =
    useState<LightboxState | null>(null);
  ...
  const [filterOutcome, setFilterOutcome] =
    useState("ALL");

  const [filterChallenge, setFilterChallenge] =
    useState("ALL");
  ...
  async function loadAll() {
    ...
    try {
      const [nextTrades, nextChallenges] =
        await Promise.all([
          fetchTrades(),
          fetchChallenges(),
        ]);
  ...
  useEffect(() => {
    if (!lightbox && !viewingTrade) return;
  ...
  const editorImages = useMemo(() => {
    const existing = editingId
      ? attachmentLightboxImages(
          editingId,
          existingAttachments,
        )
      : [];

    const queued = queuedImages.map((image) => ({
      id: image.id,
  ...
  async function loadEditorAttachments(
    tradeId: string,
  ) {
    ...
    try {
      setExistingAttachments(
        await fetchTradeAttachments(tradeId),
      );
    } finally {
  ...
  function buildInput(): TradeEditableInput {
    ...
    return {
      ...
      closedAt: hasClosedAt
        ? toIso(draft.closedAt)
        : null,

      entryPrice,
      stopPrice:
        parseOptionalNumber(draft.stopPrice),
      targetPrice:
        parseOptionalNumber(draft.targetPrice),
      exitPrice:
        parseOptionalNumber(draft.exitPrice),
  ...
  function queueFiles(files: File[]) {
    ...
    for (const file of files) {
      if (!allowed.has(file.type)) {
        setError(
          "Screenshots support JPG, PNG and WEBP only.",
        );
        return;
      }
    ...
    if (files.length > remaining) {
      setError(
        `A trade can have up to ${MAX_ATTACHMENTS_PER_TRADE} screenshots. You can add ${Math.max(
          0,
          remaining,
        )} more.`,
      );
  ...
  async function removeExistingAttachment(
    attachment: TradeAttachmentApiModel,
  ) {
    if (!editingId) return;

    const ok = window.confirm(
      `Remove screenshot "${attachment.originalFilename}"?`,
    );

    if (!ok) return;

    try {
      await deleteTradeAttachmentViaApi(
        editingId,
        attachment.id,
      );
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      if (savedTrade && !editingId) {
        ...
        try {
          await loadEditorAttachments(
            savedTrade.id,
          );
        } catch {
          // Keep the original save/upload error.
        }
      }
      ...
      setError(
        savedTrade
          ? `Trade was saved, but the screenshot step failed: ${message}`
          : message,
      );
    } finally {
      setSaving(false);
  ...
  async function remove(trade: TradeApiModel) {
    const ok = window.confirm(
      `Delete ${trade.instrument} trade from ${new Date(
        trade.openedAt,
      ).toLocaleString()}? Its screenshots will also be deleted.`,
    );
  ...
  async function openTrade(
    ...
    try {
      setViewAttachments(
        await fetchTradeAttachments(trade.id),
      );
    } catch (err) {
      setError(
        err instanceof Error
  ...
  function openViewLightbox(index: number) {
    ...
    const images = attachmentLightboxImages(
      viewingTrade.id,
      viewAttachments,
    );
  ...
  return (
    <main className={styles.page}>
        ...
        <section className={styles.panel}>
          <div className={styles.panelTitle}>
            <div>
              ...
              <small>
                P&amp;L and R are calculated by the server.
                Screenshots stay attached to this trade.
              </small>
            </div>
          ...
          >
            <div className={styles.formGrid}>
              ...
              <div
                className={`${styles.fieldWide} ${styles.screenshotField}`}
              >
                <div
                  className={styles.screenshotHeader}
                >
                  <div>
                      ...
                      (image, queuedIndex) => (
                        ...
                        >
                          ...
                          <figcaption>
                            ...
                            <button
                              type="button"
                              aria-label="Remove queued screenshot"
                              onClick={() =>
                                removeQueuedImage(
                                  image.id,
                                )
      ...
      {viewingTrade && (
        <div
          className={styles.tradeModalBackdrop}
          onMouseDown={(event) => {
            if (
              event.target ===
              event.currentTarget
        ...
        >
          <article className={styles.tradeModal}>
            ...
            <div
              className={styles.tradeModalBody}
            >
              <section
                className={styles.tradeFacts}
              >
                <Detail
                  label="Challenge"
                  value={challengeLabel(
                    viewingTrade.challengeId,
                    challenges,
                  )}
                />
              ...
              {(viewingTrade.setup ||
                viewingTrade.tags.length >
                  0 ||
                viewingTrade.notes) && (
                <section
                  className={
                    styles.tradeNarrative
                  }
                >
                  {viewingTrade.setup && (
                    <div>
              ...
              <section
                className={
                  styles.tradeScreenshots
                }
              >
                <div
                  className={
                    styles.tradeScreenshotsTitle
                  }
                >
                  <span>SCREENSHOTS</span>
                  <small>
                ...
                {viewLoading ? (
                  ...
                  >
                    No screenshots attached to
                    this trade.
                  </div>
                ) : (
                  <div
                    className={
                      styles.detailThumbnailGrid
                    }
                  >
                    {viewAttachments.map(
                      (
