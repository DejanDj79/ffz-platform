...
function FieldRow({ icon, label, accent = "cyan", children, hint }: {
  icon: string;
  label: ReactNode;
  accent?: "cyan" | "purple";
  children: ReactNode;
...
function ResultCard({ icon, title, value, sub, accent = "cyan" }: {
  icon: string;
  title: string;
  value: string;
  sub?: string;
...
export function PublicRiskCalculator() {
  ...
  const contractType = instrument === "MNQ" || instrument === "MES" ? "MICRO" : "MINI";
  ...
  const tradeVerdict = result
    ? evaluateTradeVerdict({ result, accountType })
    : null;
    ...
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="education-chip">
          <Icon name="cap" />
          <span>Free Calculator<small>No account required.</small></span>
        </div>
      </header>
      ...
      <form className="workspace" onSubmit={calculate}>
        <section className="main-panel inputs-panel">
          ...
          <div className="input-list">
            ...
            <FieldRow icon="bars" label="Instrument" hint="MNQ, MES, NQ, ES">
              <select value={instrument} onChange={(event) => setInstrument(event.target.value as InstrumentCode)}>
                {Object.values(INSTRUMENTS).map((item) => <option key={item.code} value={item.code}>{item.code}</option>)}
              </select>
            </FieldRow>

            <FieldRow icon="up" label="Entry"><input inputMode="decimal" value={entry} onChange={(event) => setEntry(event.target.value)} /></FieldRow>
            <FieldRow icon="down" label="Stop Loss" accent="purple"><input inputMode="decimal" value={stop} onChange={(event) => setStop(event.target.value)} /></FieldRow>
            <FieldRow icon="target" label={<>Optional Target <span className="optional">(optional)</span></>} accent="purple"><input inputMode="decimal" value={target} onChange={(event) => setTarget(event.target.value)} /></FieldRow>
            <FieldRow icon="shield" label="Max Risk per Trade"><div className="money-input"><span>$</span><input inputMode="decimal" value={maxRisk} onChange={(event) => setMaxRisk(event.target.value)} /></div></FieldRow>
            <FieldRow icon="money" label="Commission & Fees" hint="$ per contract round trip"><div className="money-input"><span>$</span><input inputMode="decimal" value={commissionAndFees} onChange={(event) => setCommissionAndFees(event.target.value)} /></div></FieldRow>
            <FieldRow icon="chart" label="Slippage Buffer" hint="ticks per contract"><input inputMode="decimal" value={slippageBufferTicks} onChange={(event) => setSlippageBufferTicks(event.target.value)} /></FieldRow>

            {accountType === "PROP" && <>
              <FieldRow icon="chart" label="Remaining Drawdown" accent="purple"><div className="money-input"><span>$</span><input inputMode="decimal" value={remainingDrawdown} onChange={(event) => setRemainingDrawdown(event.target.value)} /></div></FieldRow>
              <FieldRow icon="calendar" label="Remaining Daily Loss Limit" accent="purple"><div className="money-input"><span>$</span><input inputMode="decimal" value={remainingDailyLoss} onChange={(event) => setRemainingDailyLoss(event.target.value)} /></div></FieldRow>
              <FieldRow icon="wallet" label={<>Current Balance <span className="optional">(optional)</span></>}><div className="money-input"><span>$</span><input inputMode="decimal" value={currentBalance} onChange={(event) => setCurrentBalance(event.target.value)} /></div></FieldRow>
            </>}
          </div>
          ...
          <div className="input-note"><Icon name="info" /><span>Free mode uses manual values and exchange tick specifications. Sign in to load saved challenges, personal guardrails, news lockouts and Journal state.</span></div>
          <div className="input-note"><Icon name="shield" /><span>All values are editable. No trading data is stored while you use the calculator as a guest.</span></div>
        </section>
        ...
        <section className="main-panel results-panel">
          ...
          <div className="results-grid">
            <ResultCard icon="crosshair" title="STOP DISTANCE" value={`${number.format(display.stopDistancePoints)} pts`} sub={`${number.format(display.stopDistanceTicks)} ticks`} />
            <ResultCard icon="money" title="RISK PER CONTRACT" value={money.format(display.riskPerContract)} />
            <ResultCard icon="contracts" title="MAXIMUM CONTRACTS" value={String(display.maxContracts)} sub={`${contractType} • ${instrument}`} accent="purple" />
            ...
            <article className={`health-card ${healthClassName}`} tabIndex={0} aria-describedby="public-challenge-health-tooltip">
              <div className="health-title"><Icon name="pulse" />CHALLENGE HEALTH</div>
              <strong>{healthLabel}</strong>
              <span className="health-shield"><Icon name="shield" /></span>
              <span className="health-tooltip" id="public-challenge-health-tooltip" role="tooltip">{healthReason}</span>
            </article>

            <article className={`health-card ${verdictClassName}`} tabIndex={0} aria-describedby="public-trade-verdict-tooltip">
              ...
              <span className="health-tooltip" id="public-trade-verdict-tooltip" role="tooltip">
                {tradeVerdict?.reasons.join(" ") ?? "Run the calculator for the core risk verdict. Sign in to add personal rules, Journal state and live news lockouts."}
              </span>
            </article>
          </div>
          ...
          <section className="trade-summary">
            ...
            <p>Free mode calculates position size from stop distance and optional prop loss limits. Create an FFZ account to save plans and connect the calculator to your trading workspace.</p>
          </section>

          <div className="input-actions">
            <button className="primary-btn" type="button" onClick={() => { window.location.href = "/register?next=/tools/risk-calculator"; }}>
              <Icon name="briefcase" />CREATE FREE ACCOUNT
            </button>
            <button className="secondary-btn" type="button" onClick={() => { window.location.href = "/login?next=/tools/risk-calculator"; }}>
              SIGN IN
            </button>
          </div>
