...
export function DailyTradingDeskFunded() {
  ...
  const load = useCallback(async (manual = false) => {
    ...
    const [challengeResult, tradeResult, ledgerResult, calendarResult] = await Promise.allSettled([
      fetchChallenges(),
      fetchTrades(),
      fetchLedgerEntries(),
    ...
    else setError("Unable to load challenge data.");

    if (tradeResult.status === "fulfilled") setTrades(tradeResult.value);
    else setError((current) => current ?? "Unable to load Journal trades.");

    if (ledgerResult.status === "fulfilled") setLedgerEntries(ledgerResult.value);
    else setError((current) => current ?? "Unable to load payout history.");

    if (calendarResult.status === "fulfilled") setCalendar(calendarResult.value);
    else setCalendarError("Economic calendar is temporarily unavailable.");
  ...
  useEffect(() => {
    ...
    const preferred = challenges.find((challenge) => challenge.status === "FUNDED")
      ?? challenges.find((challenge) => challenge.status === "IN_PROGRESS")
      ?? challenges[0];
    setSettings((current) => ({ ...current, accountId: preferred?.id ?? "ALL" }));
  }, [challenges, hydrated, loading, settings.accountId]);
  ...
  const effectiveSelectedChallenge = useMemo(
    () => selectedChallenge
      ? effectiveChallengeAfterPayouts(selectedChallenge, ledgerEntries)
      : null,
    [ledgerEntries, selectedChallenge],
  );

  const challengeMetrics = useMemo(
    () => effectiveSelectedChallenge ? calculateChallengeMetrics(effectiveSelectedChallenge) : null,
    [effectiveSelectedChallenge],
  );

  const fundedSummary = useMemo(
    () => selectedChallenge
      ? calculateFundedPayoutSummary(selectedChallenge, trades, ledgerEntries)
      : null,
    [ledgerEntries, selectedChallenge, trades],
  );
  ...
  const tradedInstruments = useMemo(() => {
    ...
    for (const trade of daySummary.trades) counts.set(trade.instrument, (counts.get(trade.instrument) ?? 0) + 1);
  ...
  const highImpactEvents = useMemo(() => {
    ...
    return calendar.events
      .filter((event) => event.impact === "High" && (event.currency === "USD" || event.country === "US") && localDateKey(event.date) === dateKey)
        ...
        <div className={styles.rightColumn}>
          <article className={styles.panel}>
            ...
            <div className={styles.guardrailsBody}>
              ...
              {fundedSummary?.isFunded && <div className={styles.guardrailRow}><span>Next payout</span><strong className={fundedSummary.eligible ? styles.positive : ""}>{fundedSummary.eligible ? "Eligible" : `${fundedSummary.readinessPct}% ready`}</strong></div>}
              <div className={styles.guardrailRow}><span>Account health</span><strong className={challengeMetrics?.health === "DANGER" ? styles.negative : challengeMetrics?.health === "CAUTION" ? styles.cautionText : challengeMetrics ? styles.positive : ""}>{challengeMetrics?.health ?? "—"}</strong></div>
            </div>
          </article>

          <article className={styles.panel}>
            <header className={styles.panelHeader}><div><span>USD HIGH-IMPACT NEWS</span><small>{calendar?.stale ? "Cached feed · provider currently stale" : "Next Forex Factory event today"}</small></div>{nextHighEvent && <strong className={styles.nextEvent}>{countdownLabel(nextHighEvent.date, now)}</strong>}</header>
            <div className={styles.newsList}>
              {calendarError && <div className={styles.newsEmpty}>{calendarError}</div>}
              {!calendarError && !nextHighEvent && <div className={styles.newsEmpty}>{loading ? "Loading economic calendar…" : "No upcoming USD high-impact events today."}</div>}
              {nextHighEvent && (
                <div className={styles.newsItem} key={nextHighEvent.id}>
                  <div className={styles.newsTime}><strong>{formatTime(nextHighEvent.date, "America/New_York")}</strong><small>NY · {formatTime(nextHighEvent.date)} local</small></div>
                  <div className={styles.newsTitle}><strong>{nextHighEvent.title}</strong><small>Forecast {nextHighEvent.forecast ?? "—"} · Previous {nextHighEvent.previous ?? "—"}</small></div>
