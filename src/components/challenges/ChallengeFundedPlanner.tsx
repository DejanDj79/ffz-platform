...
function healthCopy(health: "SAFE" | "CAUTION" | "DANGER", funded: boolean) {
  if (health === "DANGER") return funded ? "The funded account is very close to a configured loss boundary." : "Your remaining challenge buffer is very tight.";
  if (health === "CAUTION") return funded ? "One funded-account loss buffer is getting tight." : "One of your remaining loss buffers is getting tight.";
  return funded ? "The funded account is inside the configured loss buffers." : "You're well within the configured planning buffers.";
}
...
function Field({ icon, label: fieldLabel, children, purple = false }: {
  icon: string;
  label: string;
  children: ReactNode;
  purple?: boolean;
  ...
  return (
    <div className={styles.formRow}>
      ...
      <label className={styles.formLabel}>{fieldLabel}</label>
      <div className={styles.formControl}>{children}</div>
    </div>
  );
}

function MetricCard({ icon, label: metricLabel, value, sub, purple = false, progress }: {
  icon: string;
  label: string;
  value: string;
  sub?: string;
  ...
  return (
    <div className={`${styles.metricCard} ${purple ? styles.purple : ""}`}>
      <div className={styles.metricLabel}><Icon name={icon} />{metricLabel}</div>
...
export function ChallengeFundedPlanner() {
  ...
  const loadAll = useCallback(async (migrateLegacy = false) => {
    try {
      setApiError(null);
      let [items, nextTrades, nextLedger] = await Promise.all([
        fetchChallenges(),
        fetchTrades(),
        fetchLedgerEntries(),
      ]);
      ...
      console.error("Unable to load challenge/funded data:", error);
      setApiError("Unable to load account data from the database.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const activePreset = useMemo(
    () => getPropFirmPreset(draft.rulesPresetId),
    [draft.rulesPresetId],
  );
  const payoutHistory = useMemo(
    () => payoutEntriesForChallenge(draft.id, ledgerEntries),
    [draft.id, ledgerEntries],
  );
  const fundedSummary = useMemo(
    () => calculateFundedPayoutSummary(draft, trades, ledgerEntries),
    [draft, ledgerEntries, trades],
  );
  const effectiveDraft = useMemo(
    () => effectiveChallengeAfterPayouts(draft, ledgerEntries),
    [draft, ledgerEntries],
  );
  const metrics = useMemo(
    () => calculateChallengeMetrics(effectiveDraft),
    [effectiveDraft],
  );
  ...
  function replaceSaved(saved: Challenge) {
    setDraft(saved);
    setChallenges((items) => {
  ...
  async function persist(next: Challenge) {
    const exists = challenges.some((item) => item.id === next.id);
    const saved = exists
      ? await updateChallengeViaApi(next)
      : await createChallengeViaApi(next);
    replaceSaved(saved);
    return saved;
  }
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      setApiError("Unable to save this account to the database.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function markPassed() {
    ...
    setApiError(null);
    try {
      await persist({ ...draft, status: "PASSED", phase: "EVALUATION" });
    } catch (error) {
      console.error("Unable to mark evaluation passed:", error);
      setApiError("Unable to update the evaluation status.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function createFunded() {
    ...
    setApiError(null);
    try {
      const next = createFundedAccountFromEvaluation(draft);
      const saved = await createChallengeViaApi(next);
      replaceSaved(saved);
    } catch (error) {
      console.error("Unable to create funded account:", error);
      setApiError("Unable to create the funded account.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function syncBalance() {
    ...
    setApiError(null);
    try {
      const balance = fundedSummary.journalDerivedBalance;
      await persist({
        ...draft,
        currentBalance: balance,
        highestEodBalance: Math.max(draft.highestEodBalance ?? draft.startingBalance, balance),
      });
    } catch (error) {
      console.error("Unable to sync funded balance:", error);
      setApiError("Unable to sync the Journal-derived balance.");
    } finally {
      setSaving(false);
    }
  }

  async function setPayoutPhase(requested: boolean) {
    setSaving(true);
    setApiError(null);
    try {
      await persist({
        ...draft,
        status: "FUNDED",
        phase: requested ? "PAYOUT" : "FUNDED",
      });
    } catch (error) {
      console.error("Unable to update payout phase:", error);
      setApiError("Unable to update the payout state.");
    } finally {
      setSaving(false);
    }
  }
  ...
  const isFunded = fundedSummary.isFunded;
  const goalSub = isFunded
    ? `${fundedSummary.tradingDays}${fundedSummary.payoutDaysRequired ? ` / ${fundedSummary.payoutDaysRequired}` : ""} payout days`
    : `${money.format(Math.max(0, metrics.currentPnl))} / ${money.format(draft.profitTarget)}`;
    ...
    <main className={styles.shell}>
      <header className={styles.header}>
        ...
        <div className={styles.titleWrap}>
          ...
          <h1>Challenge / Funded</h1>
        </div>
        <div className={styles.systemChip}>
          <span className={styles.systemDot} />
          <span className={styles.systemText}>{apiError ? "Database unavailable" : loading ? "Connecting..." : "Database connected"}<small>Journal + Ledger feed funded tracking</small></span>
        </div>
      </header>
      ...
      <div className={styles.workspace}>
        <form className={styles.panel} onSubmit={save}>
          ...
          {loading && <div className={styles.ruleNotice}><strong>Loading</strong><span>Reading challenges, trades and ledger entries...</span></div>}
          {activePreset && (
        ...
        <div className={styles.rightColumn}>
          <section className={styles.panel}>
            <div className={`${styles.panelTitle} ${styles.purple}`}><Icon name="chart" />ACCOUNT OVERVIEW</div>
            <div className={styles.overviewBody}>
              <div className={styles.overviewGrid}>
                <MetricCard icon="dollar" label="CURRENT BALANCE" value={money.format(draft.currentBalance)} sub={`${money.format(metrics.currentPnl)} from account start`} />
                <MetricCard
                  icon="target"
                  label={isFunded ? "PAYOUT AVAILABLE" : "PROFIT TARGET REMAINING"}
                  value={isFunded ? money.format(fundedSummary.grossPayoutAvailable) : money.format(metrics.profitTargetRemaining)}
                  sub={isFunded ? `Est. take-home ${money.format(fundedSummary.estimatedTraderPayout)}` : `${number.format(100 - metrics.targetProgressPct)}% to go`}
                />
                ...
                <MetricCard
                  icon="target"
                  label={isFunded ? "PAYOUT READINESS" : "PROGRESS TO TARGET"}
                  value={`${number.format(isFunded ? fundedSummary.readinessPct : metrics.targetProgressPct)}%`}
                  sub={goalSub}
                  progress={isFunded ? fundedSummary.readinessPct : metrics.targetProgressPct}
                />
                <MetricCard
                  icon="calendar"
                  label={isFunded ? "CYCLE TRADING DAYS" : "DAYS TRADED"}
                  value={String(isFunded ? fundedSummary.tradingDays : draft.daysTraded)}
                  sub={isFunded ? (fundedSummary.payoutDaysRequired ? `${fundedSummary.payoutDaysRequired} required for payout` : "No payout-day rule") : (draft.minimumTradingDays ? `Min. required: ${draft.minimumTradingDays}` : "No minimum configured")}
                  purple
                />
                <MetricCard icon="flag" label="STATUS" value={label(draft.status)} sub={`Phase: ${label(draft.phase)}`} />
                <MetricCard icon="dollar" label="REAL MONEY COST" value={money.format(metrics.realMoneyCost)} sub={draft.resetsUsed ? `${draft.resetsUsed} reset(s) included` : isFunded ? `${fundedSummary.payoutCount} received payout(s)` : "Challenge fee only"} purple />

                <div className={styles.healthCard}>
                  <div className={styles.healthInner}>
                    <div className={styles.healthLabel}><Icon name="pulse" />ACCOUNT HEALTH</div>
                    ...
                    <div className={styles.healthText}>{healthCopy(metrics.health, isFunded)}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <FundedLifecyclePanel
            challenge={draft}
            metrics={metrics}
            funded={fundedSummary}
            payouts={payoutHistory}
            saving={saving}
            onMarkPassed={() => void markPassed()}
            onCreateFunded={() => void createFunded()}
      ...
      <footer className={styles.footer}>Backend v1.3 · Challenge, Journal and Ledger data stay in PostgreSQL.</footer>
    </main>
  );
}
